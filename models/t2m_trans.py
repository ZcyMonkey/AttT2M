...
class Text2Motion_Cross_Transformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_layers_cross=2):
        super().__init__()
        self.trans_base = CrossAttTransBase(num_vq, embed_dim, clip_dim, block_size, num_layers, n_head, drop_out_rate, fc_rate,num_layers_cross)
        self.trans_head = CrossCondTransHead(num_vq, embed_dim, block_size, num_layers, n_head, drop_out_rate, fc_rate)
        self.block_size = block_size
        self.num_vq = num_vq
    ...
    def forward(self, idxs, clip_feature,word_emb):
        feat = self.trans_base(idxs, clip_feature,word_emb)
        logits = self.trans_head(feat)
        return logits

    def sample(self, clip_feature, word_emb,if_categorial=False):
        for k in range(self.block_size):
            if k == 0:
                x = []
            else:
                x = xs
            logits = self.forward(x, clip_feature,word_emb)
            logits = logits[:, -1, :]
            probs = F.softmax(logits, dim=-1)
            if if_categorial:
                dist = Categorical(probs)
...
class CrossAttTransBase(nn.Module):
    ...
    def forward(self, idx, clip_feature,word_emb):
        if len(idx) == 0:
